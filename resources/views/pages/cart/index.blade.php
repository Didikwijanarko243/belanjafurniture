...
@section('content')
    <div class="max-w-5xl mx-auto px-6 lg:px-8 py-12">
        ...
        @if ($cart->items->isEmpty())
            <div
                class="flex flex-col items-center justify-center text-center py-24 border border-dashed border-walnut/20 rounded-lg">
                <svg xmlns="http://www.w3.org/2000/svg" class="w-10 h-10 text-walnut/30 mb-4" viewBox="0 0 24 24" fill="none"
                ...
                <a href="{{ route('produk.index') }}"
                    class="text-sm text-walnut font-medium hover:text-walnut-dark transition-colors">
                    ...
                    @foreach ($cart->items as $item)
                        <div class="flex gap-4 border border-walnut/10 rounded-lg p-4">
                            <div class="w-20 h-20 rounded-md overflow-hidden bg-canvas shrink-0">
                                ...
                                @if ($image)
                                    <img src="{{ Storage::url($image->image_path) }}" alt="{{ $item->product->name }}"
                                        class="w-full h-full object-cover">
                                @endif
                            </div>

                            <div class="flex-1">
                                <a href="{{ route('produk.show', $item->product->slug) }}"
                                    class="font-display text-base font-medium text-ink hover:text-walnut transition-colors">
                                    {{ $item->product->name }}
                                </a>
                                @if ($item->variant)
                                ...
                                <div class="flex items-center gap-4 mt-3">
                                    <form method="POST" action="{{ route('keranjang.update', $item) }}"
                                        ...
                                        <button type="submit"
                                            class="text-xs text-walnut hover:text-walnut-dark transition-colors">Update</button>
                                    </form>
                                    ...
                                    <form method="POST" action="{{ route('keranjang.destroy', $item) }}">
                                        ...
                                        @method('DELETE')
                                        <button type="submit"
                                            class="text-xs text-rust hover:text-rust-dark transition-colors">Hapus</button>
                                    </form>
                                </div>
                            </div>
                ...
                <div class="lg:col-span-1">
                    <div class="border border-walnut/10 rounded-lg p-6 sticky top-28">
                        ...
                        <form action="{{ route('cart.whatsapp') }}" method="POST">
                            @csrf
                            <button type="submit"
                                class="w-full inline-flex items-center justify-center px-6 py-3 rounded-lg bg-rust text-canvas text-sm font-semibold hover:bg-rust/90 transition-colors">
