...
@section('content')
    <div class="max-w-7xl mx-auto px-6 lg:px-8 py-12">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-12" id="product-detail" data-product-id="{{ $product->id }}"
            data-product-name="{{ $product->name }}" data-base-price="{{ $product->final_price }}"
            data-stock="{{ $product->stock }}" data-whatsapp-number="{{ config('shop.whatsapp_number') }}">
            {{-- ============ GALERI ============ --}}
            <div>
                <div class="aspect-square rounded-lg overflow-hidden bg-canvas border border-walnut/10">
                    @if ($product->images->isNotEmpty())
                        <img id="gallery-main-image" src="{{ Storage::url($product->images->first()->image_path) }}"
                        ...
                        @foreach ($product->images as $image)
                            <button type="button"
                                ...
                                <img src="{{ Storage::url($image->image_path) }}"
                                    alt="{{ $image->alt_text ?? $product->name }}" class="w-full h-full object-cover">
                            </button>
                        @endforeach
                    </div>
                @endif
            ...
            <div>
                @if ($product->category)
                    <span
                        class="text-xs uppercase tracking-wide text-sage font-medium">{{ $product->category->name }}</span>
                @endif
                ...
                <div class="flex items-center gap-3 mt-3">
                    @if ($product->average_rating > 0)
                        <div class="flex items-center gap-1.5 text-sm text-ink/60">
                            ...
                            <span>{{ $product->average_rating }} ({{ $product->review_count }} ulasan)</span>
                        </div>
                    @endif
                    @if ($product->sku)
                        ...
                        <div class="mb-5">
                            ...
                            <div class="flex flex-wrap gap-2" id="variant-color-group">
                                @foreach ($colors as $color)
                                    <button type="button" data-variant-color="{{ $color }}"
                        ...
                        <div class="mb-5">
                            ...
                            <div class="flex flex-wrap gap-2" id="variant-size-group">
                                @foreach ($sizes as $size)
                                    <button type="button" data-variant-size="{{ $size }}"
                                        class="variant-option px-4 py-2 rounded-md border border-walnut/20 text-sm text-ink/70 hover:border-walnut transition-colors">
                                        {{ $size }}
                                    </button>
                                @endforeach
                ...
                <div class="mb-6">
                    ...
                    <div class="inline-flex items-center border border-walnut/20 rounded-md">
                        <button type="button" id="qty-decrease"
                            class="w-10 h-10 flex items-center justify-center text-ink/60 hover:text-walnut transition-colors"
                ...
                <div class="flex flex-col sm:flex-row gap-3">
                    ...
                    <button type="button"
                        ...
                        data-product-id="{{ $product->id }}" aria-label="Simpan ke wishlist"
                    ...
                    <form id="form-whatsapp-order" method="POST"
                        action="{{ route('produk.whatsapp', $product->slug) }}" class="flex-1">
                        @csrf
                        <input type="hidden" name="product_variant_id" id="input-variant-id" value="">
                        <input type="hidden" name="quantity" id="input-whatsapp-quantity" value="1">
                        <button type="submit"
                            class="w-full flex items-center justify-center gap-2 bg-sage text-canvas font-medium text-sm py-3 rounded-md hover:opacity-90 transition-opacity disabled:opacity-40 disabled:cursor-not-allowed">
                    ...
                    {{-- <a id="btn-whatsapp-order" href="#" target="_blank" rel="noopener"
        ...
        {{-- resources/views/produk/show.blade.php — tambahkan di bagian bawah detail produk --}}

        <section class="mt-12 border-t border-walnut/10 pt-8">
            <div class="flex items-center gap-3 mb-6">
                <h2 class="font-fraunces text-2xl text-walnut">Ulasan Pembeli</h2>
                @if ($product->review_count > 0)
                    <x-star-rating :rating="$product->average_rating" />
                    <span class="text-sm text-walnut/60">
                        {{ $product->average_rating }} ({{ $product->review_count }} ulasan)
                    </span>
                @endif
            </div>

            @forelse($product->approvedReviews as $review)
                <div class="mb-6 border-b border-walnut/5 pb-6">
                    <div class="flex items-center justify-between">
                        <span class="font-medium text-walnut">{{ $review->reviewer_name }}</span>
                        <x-star-rating :rating="$review->rating" size="sm" />
                    </div>
                    <p class="text-sm text-walnut/50 mt-1">{{ $review->created_at->translatedFormat('d F Y') }}</p>
                    @if ($review->comment)
                        <p class="mt-2 text-walnut/80">{{ $review->comment }}</p>
                    @endif
                </div>
            @empty
                <p class="text-walnut/50">Belum ada ulasan untuk produk ini.</p>
            @endforelse
        </section>
    </div>
@endsection
